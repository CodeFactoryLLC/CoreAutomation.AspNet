...
using CoreAutomation.AspNet.Commands.ExplorerCommands.SourceCode.Logic;

namespace CoreAutomation.AspNet.Commands.ExplorerCommands.SourceCode
{
    /// <summary>
    /// Code factory command for automation of a C# document when selected from a project in solution explorer.
    /// </summary>
    public class AddMembersCommand : CSharpSourceCommandBase
    {
        private static readonly string commandTitle = "Add Missing Members";
        private static readonly string commandDescription = "Adds missing interface members from the target class.";

#pragma warning disable CS1998

        /// <inheritdoc />
        public AddMembersCommand(ILogger logger, IVsActions vsActions) : base(logger, vsActions, commandTitle, commandDescription)
        {
            //Intentionally blank
        }
        ...
        public override async Task<bool> EnableCommandAsync(VsCSharpSource result)
        {
            ...
            try
            {
                ...
                if(isEnabled)
                {
                    ...
                    if (hasLogging & hasCDF) isEnabled = false;
                }
        ...
        public override async Task ExecuteCommandAsync(VsCSharpSource result)
        {
            try
            {
                ...
                var enableLogging = await hostingProject.HasReferenceLibraryAsync(AspNetCoreConstants.MicrosoftExtensionLibraryForLoggingName);
